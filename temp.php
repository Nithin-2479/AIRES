...
function recentJobDescriptions($conn) {
    try {
        $query = "SELECT joborder_id as id, title, date_created 
                  FROM joborder 
                  WHERE status = 'Active'
                  ORDER BY date_created DESC 
                  LIMIT 10";
        $json_output = $conn->query($query);
        
        if (!$json_output) {
            throw new Exception("Query failed: " . $conn->error);
        }
        
        return $json_output->fetch_all(MYSQLI_ASSOC);
    } catch (Exception $e) {
        error_log("Error fetching recent job descriptions: " . $e->getMessage());
        return [];
    }
}
$recentJobDescriptions = recentJobDescriptions($conn);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['job_description']) && isset($_POST['match_percentage'])) {
        ...
        if ($_POST['job_description_option'] == 'select') {
            $jobId = $_POST['recent_job_description'];
            $query = "SELECT description FROM joborder WHERE joborder_id = ?";
        ...
        } elseif ($_POST['job_description_option'] == 'upload') {
            // Handle file upload and extract text (you may need to implement this)
            $jobDescription = "Extracted text from uploaded file";
        }
...
<body>
    <div class="container">
        ...
        <div>
                ...
                <input type="radio" id="upload_jd" name="job_description_option" value="upload"value="<?php echo $_POST['job_description']; ?>">
                <label for="upload_jd">Upload Document</label>
                <input type="radio" id="select_jd" name="job_description_option" value="select"value="<?php echo $_POST['job_description']; ?>">
            ...
            <div id="select_jd_section" style="display: none;">
                <select name="recent_job_description" id="recent_job_description">
                    <option value="">Select a recent job description</option>
                        <?php foreach ($recentJobDescriptions as $job): ?>
                    <option value="<?php echo htmlspecialchars($job['id']); ?>">
                        <?php echo htmlspecialchars($job['title']) . ' (Created: ' . date('Y-m-d', strtotime($job['date_created'])) . ')'; ?>
                    <option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    <script>
        ...
        document.getElementById('recent_job_description').addEventListener('change', function() {
            var selectedJobId = this.value;
            if (selectedJobId) {
        // You can add AJAX call here to fetch full job description if needed
